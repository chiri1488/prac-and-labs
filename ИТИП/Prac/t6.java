...
public class t6{
    ...
    System.out.println(collect("pneumonoultramicroscopicsilicovolcanoconiosis", 15));
        ...
        //~ ПУСТОЙ МАССИВ ТАК КАК n > str
    System.out.println(collect("abc", 5) + "\n");
        //* [""] 
    //& 3
    ...
    public static String nicoCipher(String message, String key) {
        ...
        String new_message = message + " ".repeat(extraSpaces);
    
        Integer[] order = IntStream.range(0, keyLength)
                                   .boxed()
                                   .sorted(Comparator.comparingInt(key::charAt))
                                   .toArray(Integer[]::new);
    
        return IntStream.range(0, new_message.length())
                        .mapToObj(i -> String.valueOf(new_message
                            .charAt((i / keyLength) * keyLength + order[i % keyLength]))
                        )
    ...
    public static String fractions(String str) {
        ...
        if (start != -1) {
            String repeating = str.substring(start + 1, str.length() - 1);
            str = str.substring(0, start) + repeating.repeat(9 - repeating.length());
        }
